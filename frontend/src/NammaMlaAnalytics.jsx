...
export default function NammaMlaAnalytics({ adminToken, API_BASE, showNotification, readOnly }) {
  ...
  const [drillModal, setDrillModal] = useState({ open: false, filterName: '', filterValue: '', complaints: [], page: 1, total: 0, loading: false });
  const [drillExpandedRow, setDrillExpandedRow] = useState(null);
  ...
  // Priority Complaint Queue State
  const [pqComplaints, setPqComplaints] = useState([]);
  const [pqTotal, setPqTotal] = useState(0);
  const [pqPage, setPqPage] = useState(1);
  const [pqLimit] = useState(15);
  const [pqLoading, setPqLoading] = useState(false);
  const [pqSearch, setPqSearch] = useState('');
  const [pqExpandedRow, setPqExpandedRow] = useState(null);
  const [pqFiltersExpanded, setPqFiltersExpanded] = useState(false);
  const [pqFilters, setPqFilters] = useState({
    priority: '',
    status: '',
    category: '',
    assignee: '',
  ...
  // Load Priority Complaint Queue
  const fetchPriorityQueue = async () => {
    if (!adminToken) return;
    setPqLoading(true);
    try {
      const queryParams = new URLSearchParams();
      queryParams.append('page', pqPage);
      queryParams.append('limit', pqLimit);
      queryParams.append('sort', 'priority_queue');
      if (pqSearch) {
        queryParams.append('search', pqSearch);
      }
      ...
      // Apply local priority queue quick filters (these override dashboard filters if set)
      if (pqFilters.priority) queryParams.set('priority', pqFilters.priority);
      if (pqFilters.status) queryParams.set('status', pqFilters.status);
      if (pqFilters.category) queryParams.set('category', pqFilters.category);
      if (pqFilters.assignee) queryParams.set('assignee', pqFilters.assignee);
      if (pqFilters.ward) queryParams.set('ward_number', pqFilters.ward);

      const res = await fetch(`${API_BASE}/admin/namma-mla/complaints?${queryParams.toString()}`, {
        headers: { 'Authorization': `Bearer ${adminToken}` }
      });
      if (!res.ok) throw new Error('Failed to fetch priority complaints.');
      const result = await res.json();
      setPqComplaints(result.complaints || []);
      setPqTotal(result.total || 0);
    } catch (err) {
      showNotification(err.message || 'Error loading priority queue.', 'error');
    } finally {
      setPqLoading(false);
    }
  };

  // Export current view of the Priority Queue
  const handleExportPriorityQueue = async (format) => {
    try {
      const queryParams = new URLSearchParams();
      queryParams.append('page', '1');
      queryParams.append('limit', '1000000'); // Fetch all matching complaints
      queryParams.append('sort', 'priority_queue');
      if (pqSearch) queryParams.append('search', pqSearch);
      ...
      // Build CSV content
      const headers = ["Complaint ID", "Category", "Ward", "Status", "Assignee", "Priority", "Description", "Citizen Name", "Mobile Number", "Date Raised"];
      const csvRows = [headers.join(",")];

      complaints.forEach(c => {
        const row = [
          `"${(c.complaint_id || '').replace(/"/g, '""')}"`,
          `"${(c.category || '').replace(/"/g, '""')}"`,
          `"Ward ${c.ward_number || ''}"`,
          `"${(c.status || '').replace(/"/g, '""')}"`,
          `"${(c.assignee || 'Unassigned').replace(/"/g, '""')}"`,
      ...
      const csvContent = "\ufeff" + csvRows.join("\n"); // prepending BOM for proper Excel encoding
      ...
      link.setAttribute("download", `namma_mla_priority_queue_export.${format === 'csv' ? 'csv' : 'xls'}`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      showNotification(`Exported current view as ${format.toUpperCase()} successfully.`, 'success');
    } catch (err) {
      showNotification('Error exporting priority queue data.', 'error');
    }
  };
  ...
  // Run when priority queue filters, page, or search query change
  useEffect(() => {
    if (activeTab === 'dashboard') {
      fetchPriorityQueue();
    }
  }, [activeTab, dashboardFilters, pqFilters, pqPage, pqSearch]);
  ...
  const handleOpenDrillDown = async (field, val, labelName) => {
    ...
    setDrillModal({ open: true, filterName: labelName, filterValue: displayVal, complaints: [], page: 1, total: 0, loading: true });
    setDrillExpandedRow(null);
  ...
  const fetchDrillComplaints = async (field, val, pageNo) => {
    ...
    try {
      ...
      // Override or append target field filters
      if (field) {
        if (typeof field === 'object' && typeof val === 'object') {
          Object.entries(field).forEach(([fK, fV]) => {
            if (fV) queryParams.set(fK, fV);
          });
        } else {
          ...
          <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
            ...
            {data && data.kpis.total_complaints > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                        ...
                        <tbody>
                          {matrixRows.map((row, idx) => (
                            <tr key={row.category} style={{ borderBottom: '1px solid #F1F5F9', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#F8FAFC' }}>
                              <td
                                style={{ padding: '8px 12px', fontWeight: '600', color: '#2563EB', cursor: 'pointer', textDecoration: 'underline' }}
                                onClick={() => handleOpenDrillDown({ category: row.category }, { category: row.category }, `${row.category} - All Complaints`)}
                              >
                                {row.category}
                              </td>
                              <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                                <span
                                  className="badge"
                                  style={{ backgroundColor: '#FFEDD5', color: '#EA580C', fontWeight: '700', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75rem', cursor: row.open > 0 ? 'pointer' : 'default' }}
                                  onClick={() => row.open > 0 && handleOpenDrillDown({ category: row.category, status: 'Open' }, { category: row.category, status: 'Open' }, `${row.category} - Open Complaints`)}
                                >
                                  {row.open}
                                </span>
                              </td>
                              <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                                <span
                                  className="badge"
                                  style={{ backgroundColor: '#FEF3C7', color: '#D97706', fontWeight: '700', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75rem', cursor: row.pending > 0 ? 'pointer' : 'default' }}
                                  onClick={() => row.pending > 0 && handleOpenDrillDown({ category: row.category, status: 'Pending' }, { category: row.category, status: 'Pending' }, `${row.category} - Pending Complaints`)}
                                >
                                  {row.pending}
                                </span>
                              </td>
                              <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                                <span
                                  className="badge"
                                  style={{ backgroundColor: '#DCFCE7', color: '#15803D', fontWeight: '700', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75rem', cursor: row.closed > 0 ? 'pointer' : 'default' }}
                                  onClick={() => row.closed > 0 && handleOpenDrillDown({ category: row.category, status: 'Resolved' }, { category: row.category, status: 'Resolved' }, `${row.category} - Closed Complaints`)}
                                >
                                  {row.closed}
                                </span>
                              </td>
                              <td
                                style={{ padding: '8px 12px', fontWeight: '700', color: '#2563EB', textAlign: 'right', cursor: row.total > 0 ? 'pointer' : 'underline' }}
                                onClick={() => row.total > 0 && handleOpenDrillDown({ category: row.category }, { category: row.category }, `${row.category} - All Complaints`)}
                              >
                                {row.total}
                              </td>
                            </tr>
                          ...
                          <tr style={{ borderTop: '2px solid #E2E8F0', borderBottom: '2px solid #E2E8F0', backgroundColor: '#F1F5F9', fontWeight: '800' }}>
                            <td
                              style={{ padding: '10px 12px', color: '#2563EB', cursor: 'pointer', textDecoration: 'underline' }}
                              onClick={() => handleOpenDrillDown({}, {}, 'All Complaints')}
                            >
                              TOTAL
                            </td>
                            <td
                              style={{ padding: '10px 12px', textAlign: 'right', color: '#EA580C', cursor: totalRow.open > 0 ? 'pointer' : 'default' }}
                              onClick={() => totalRow.open > 0 && handleOpenDrillDown({ status: 'Open' }, { status: 'Open' }, 'All Open Complaints')}
                            >
                              {totalRow.open}
                            </td>
                            <td
                              style={{ padding: '10px 12px', textAlign: 'right', color: '#D97706', cursor: totalRow.pending > 0 ? 'pointer' : 'default' }}
                              onClick={() => totalRow.pending > 0 && handleOpenDrillDown({ status: 'Pending' }, { status: 'Pending' }, 'All Pending Complaints')}
                            >
                              {totalRow.pending}
                            </td>
                            <td
                              style={{ padding: '10px 12px', textAlign: 'right', color: '#15803D', cursor: totalRow.closed > 0 ? 'pointer' : 'default' }}
                              onClick={() => totalRow.closed > 0 && handleOpenDrillDown({ status: 'Resolved' }, { status: 'Resolved' }, 'All Closed Complaints')}
                            >
                              {totalRow.closed}
                            </td>
                            <td
                              style={{ padding: '10px 12px', textAlign: 'right', color: '#2563EB', cursor: totalRow.total > 0 ? 'pointer' : 'underline' }}
                              onClick={() => totalRow.total > 0 && handleOpenDrillDown({}, {}, 'All Complaints')}
                            >
                              {totalRow.total}
                            </td>
                          </tr>
                  ...
                  <div className="card" style={{ padding: '20px', borderRadius: '16px', boxShadow: '0 4px 20px -2px rgba(0,0,0,0.05)', backgroundColor: '#ffffff' }}>
                    ...
                    <div style={{ height: '300px' }}>
                      <EChart
                        ...
                        onEvents={{
                          click: (params) => {
                            const dbName = params.name.toUpperCase();
                            handleOpenDrillDown('priority', dbName, `Priority: ${params.name}`);
                          },
                          legendselectchanged: (params) => {
                            if (params.name) {
                              const baseName = params.name.split(':')[0].trim();
                              if (['critical', 'high', 'medium'].includes(baseName.toLowerCase())) {
                                const dbName = baseName.toUpperCase();
                                handleOpenDrillDown('priority', dbName, `Priority: ${baseName}`);
                              }
                            }
                          }
                        }}
                ...
                {/* Row 3: Priority Complaint Queue */}
                <div className="card" style={{ padding: '24px', borderRadius: '16px', boxShadow: '0 4px 20px -2px rgba(0,0,0,0.05)', backgroundColor: '#ffffff', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div>
                    <h3 style={{ color: '#0F172A', fontSize: '1.25rem', margin: 0, fontWeight: '800', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <AlertTriangle size={22} style={{ color: '#DC2626' }} />
                      Priority Complaint Queue
                    </h3>
                    <p style={{ color: '#64748B', fontSize: '0.85rem', margin: '4px 0 0 0' }}>
                      Complaints requiring immediate administrative attention, automatically sorted by priority.
                    </p>
                  </div>

                  {/* Queue Controls: Search & Filters */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '16px', backgroundColor: '#F8FAFC', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', flexGrow: 1, minWidth: '280px' }}>
                        <div style={{ position: 'relative', flexGrow: 1 }}>
                          ...
                          <input
                            type="text"
                            placeholder="Search ID, description, assignee, ward..."
                            value={pqSearch}
                            onChange={(e) => { setPqSearch(e.target.value); setPqPage(1); }}
                            style={{ width: '100%', padding: '10px 12px 10px 36px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.85rem', outline: 'none', backgroundColor: '#ffffff' }}
                          />
                        </div>

                        <button
                          onClick={() => setPqFiltersExpanded(!pqFiltersExpanded)}
                          className="btn"
                          style={{
                            padding: '10px 16px',
                            borderRadius: '8px',
                            ...
                            backgroundColor: pqFiltersExpanded ? '#2563EB' : '#ffffff',
                            color: pqFiltersExpanded ? '#ffffff' : '#0F172A',
                            fontWeight: '600',
                            fontSize: '0.85rem',
                            transition: 'all 0.2s'
                          }}
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <button
                          className="btn btn-secondary"
                          style={{ padding: '10px 16px', borderRadius: '8px', fontWeight: '600', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}
                          onClick={() => handleExportPriorityQueue('csv')}
                        >
                        ...
                        <button
                          className="btn btn-secondary"
                          style={{ padding: '10px 16px', borderRadius: '8px', fontWeight: '600', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}
                          onClick={() => handleExportPriorityQueue('excel')}
                        >
                    ...
                    {pqFiltersExpanded && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', padding: '16px', backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #CBD5E1', marginTop: '10px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>

                        {/* Priority Filter */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <label style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B' }}>Priority</label>
                          <select
                            value={pqFilters.priority}
                            onChange={(e) => { setPqFilters(prev => ({ ...prev, priority: e.target.value })); setPqPage(1); }}
                            style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.85rem', fontWeight: '500', outline: 'none', backgroundColor: '#ffffff', color: '#0F172A', minWidth: '130px' }}
                          >
                            <option value="">All Priorities</option>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          ...
                          <select
                            value={pqFilters.status}
                            onChange={(e) => { setPqFilters(prev => ({ ...prev, status: e.target.value })); setPqPage(1); }}
                            style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.85rem', fontWeight: '500', outline: 'none', backgroundColor: '#ffffff', color: '#0F172A', minWidth: '130px' }}
                          >
                            <option value="">All Statuses</option>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          ...
                          <select
                            value={pqFilters.category}
                            onChange={(e) => { setPqFilters(prev => ({ ...prev, category: e.target.value })); setPqPage(1); }}
                            style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.85rem', fontWeight: '500', outline: 'none', backgroundColor: '#ffffff', color: '#0F172A', maxWidth: '180px' }}
                          >
                            <option value="">All Categories</option>
                            {filterOptions.categories.map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        ...
                        {/* Assignee Filter */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <label style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B' }}>Assignee</label>
                          <select
                            value={pqFilters.assignee}
                            onChange={(e) => { setPqFilters(prev => ({ ...prev, assignee: e.target.value })); setPqPage(1); }}
                            style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.85rem', fontWeight: '500', outline: 'none', backgroundColor: '#ffffff', color: '#0F172A', maxWidth: '160px' }}
                          >
                        ...
                        {/* Ward Filter */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <label style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B' }}>Ward</label>
                          <select
                            value={pqFilters.ward}
                            onChange={(e) => { setPqFilters(prev => ({ ...prev, ward: e.target.value })); setPqPage(1); }}
                            style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.85rem', fontWeight: '500', outline: 'none', backgroundColor: '#ffffff', color: '#0F172A' }}
                          >
                            <option value="">All Wards</option>
                            {filterOptions.wards.map(w => <option key={w} value={w}>Ward {w}</option>)}
                          </select>
                  ...
                  {/* Queue Table */}
                  <div style={{ position: 'relative', overflowX: 'auto', border: '1px solid #E2E8F0', borderRadius: '12px', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.02)' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'left' }}>
                      <thead style={{ position: 'sticky', top: 0, backgroundColor: '#F8FAFC', zIndex: 10, borderBottom: '2px solid #E2E8F0' }}>
                        <tr>
                          ...
                          <th style={{ padding: '12px 16px', fontWeight: '700', color: '#475569' }}>Ward</th>
                          <th style={{ padding: '12px 16px', fontWeight: '700', color: '#475569' }}>Status</th>
                          <th style={{ padding: '12px 16px', fontWeight: '700', color: '#475569' }}>Assignee</th>
                        ...
                        {pqLoading ? (
                          <tr>
                            <td colSpan={7} style={{ padding: '40px', textAlign: 'center', color: '#64748B' }}>
                              <RefreshCw size={24} className="loading-spinner" style={{ margin: '0 auto 12px' }} />
                              <div>Loading priority complaints...</div>
                            </td>
                          </tr>
                        ) : pqComplaints.length === 0 ? (
                          <tr>
                            <td colSpan={7} style={{ padding: '60px 40px', textAlign: 'center' }}>
                              <AlertTriangle size={48} style={{ color: '#94A3B8', margin: '0 auto 16px' }} />
                              <h4 style={{ color: '#0F172A', margin: '0 0 4px 0', fontWeight: '700' }}>No complaints found</h4>
                              <p style={{ color: '#64748B', margin: 0 }}>No complaints matching the selected filters.</p>
                            </td>
                          </tr>
                        ) : (
                          pqComplaints.map((item) => {
                            const isExpanded = pqExpandedRow === item.complaint_id;

                            // Border colors matching priority
                            const priorityBorders = {
                              'CRITICAL': '5px solid #DC2626',
                              'HIGH': '5px solid #EA580C',
                              'MEDIUM': '5px solid #EAB308',
                              'LOW': '5px solid #22C55E'
                            };
                            const leftBorder = priorityBorders[item.priority?.toUpperCase()] || '5px solid #CBD5E1';

                            // Priority Badge
                            const getPriorityBadge = (p) => {
                              const pColors = {
                                'critical': { bg: '#FEE2E2', text: '#991B1B', emoji: '🔴' },
                                'high': { bg: '#FFEDD5', text: '#9A3412', emoji: '🟠' },
                                'medium': { bg: '#FEF3C7', text: '#92400E', emoji: '🟡' },
                                'low': { bg: '#DCFCE7', text: '#166534', emoji: '🟢' }
                              };
                              const norm = p?.toLowerCase() || 'low';
                              const cfg = pColors[norm] || pColors['low'];
                              return (
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', backgroundColor: cfg.bg, color: cfg.text, padding: '4px 10px', borderRadius: '9999px', fontWeight: '700', fontSize: '0.75rem', textTransform: 'capitalize' }}>
                                  <span>{cfg.emoji}</span> {norm}
                                </span>
                              );
                            };
                            ...
                            const getStatusBadge = (s) => {
                              const sColors = {
                                'resolved': { bg: '#DCFCE7', text: '#166534', emoji: '🟢' },
                                'pending': { bg: '#FEF3C7', text: '#92400E', emoji: '🟡' },
                                'in progress': { bg: '#DBEAFE', text: '#1E40AF', emoji: '🔵' },
                                'assigned': { bg: '#E0F2FE', text: '#0369A1', emoji: '🔵' },
                                'open': { bg: '#FEE2E2', text: '#991B1B', emoji: '🔴' }
                              };
                              const norm = s?.toLowerCase() || 'open';
                              const cfg = sColors[norm] || sColors['open'];
                              const displayStatus = s === 'Resolved' ? 'Closed' : s;
                              return (
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', backgroundColor: cfg.bg, color: cfg.text, padding: '4px 10px', borderRadius: '9999px', fontWeight: '700', fontSize: '0.75rem' }}>
                                  <span>{cfg.emoji}</span> {displayStatus}
                                </span>
                              );
                            };
                            ...
                            return (
                              <React.Fragment key={item.complaint_id}>
                                <tr
                                  onClick={() => setPqExpandedRow(isExpanded ? null : item.complaint_id)}
                                  style={{
                                  ...
                                  className="pq-row"
                                >
                                  <td style={{ padding: '14px 16px', fontWeight: '700', color: '#2563EB' }}>{item.complaint_id}</td>
                                  <td style={{ padding: '14px 16px', color: '#334155', maxWidth: '300px' }} title={desc}>
                                    {truncatedDesc}
                                  </td>
                                  <td style={{ padding: '14px 16px', fontWeight: '500', color: '#475569' }}>
                                    {item.category?.replace('_', ' ')}
                                  </td>
                                  <td style={{ padding: '14px 16px', fontWeight: '600', color: '#1E293B' }}>Ward {item.ward_number}</td>
                                  <td style={{ padding: '14px 16px' }}>{getStatusBadge(item.status)}</td>
                                  <td style={{ padding: '14px 16px', color: '#475569', fontWeight: '500' }}>{item.assignee || 'Unassigned'}</td>
                                  <td style={{ padding: '14px 16px' }}>{getPriorityBadge(item.priority)}</td>
                                </tr>

                                {/* Expanded Row */}
                                {isExpanded && (
                                  <tr style={{ borderBottom: '1px solid #E2E8F0', backgroundColor: '#F8FAFC', borderLeft: leftBorder }}>
                                    <td colSpan={7} style={{ padding: '0 24px 20px 24px' }}>
                                      <div style={{
                                        display: 'grid',
                                        ...
                                        <div style={{ gridColumn: '1 / -1' }}>
                                          <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '6px' }}>Full Description</div>
                                          <div style={{ fontSize: '0.9rem', color: '#0F172A', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>{item.description || 'No description provided.'}</div>
                                        </div>

                                        <div>
                                          <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '6px' }}>Citizen Name</div>
                                          <div style={{ fontSize: '0.9rem', fontWeight: '600', color: '#1E293B' }}>{item.citizen_name || 'N/A'}</div>
                                        </div>

                                        <div>
                                          <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '6px' }}>Mobile Number</div>
                                        ...
                                        <div>
                                          <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '6px' }}>Date Raised</div>
                                          <div style={{ fontSize: '0.9rem', fontWeight: '600', color: '#1E293B' }}>
                                            {item.created_at ? new Date(item.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                                          </div>
                                        </div>

                                        <div>
                                          <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '6px' }}>Last Updated</div>
                                          <div style={{ fontSize: '0.9rem', fontWeight: '600', color: '#1E293B' }}>
                                            {item.resolved_at ? new Date(item.resolved_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) :
                                              item.imported_at ? new Date(item.imported_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}
                                          </div>
                                        </div>

                                        {item.resolution_note && (
                                          <div style={{ gridColumn: '1 / -1', borderTop: '1px solid #F1F5F9', paddingTop: '12px' }}>
                                            <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '6px' }}>Resolution Remarks</div>
                                            <div style={{ fontSize: '0.9rem', color: '#16A34A', fontWeight: '500', lineHeight: '1.5' }}>{item.resolution_note}</div>
                                          </div>
                                        )}
                                      </div>
                                    </td>
                  ...
                  {/* Queue Pagination */}
                  {!pqLoading && pqComplaints.length > 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 4px' }}>
                      <div style={{ fontSize: '0.8rem', color: '#64748B' }}>
                        Showing <strong>{(pqPage - 1) * pqLimit + 1}</strong> to <strong>{Math.min(pqPage * pqLimit, pqTotal)}</strong> of <strong>{pqTotal}</strong> priority complaints
                      </div>
                      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                        ...
                        {/* Page Numbers */}
                        {Array.from({ length: Math.ceil(pqTotal / pqLimit) }, (_, i) => i + 1)
                          .filter(p => p === 1 || p === Math.ceil(pqTotal / pqLimit) || Math.abs(p - pqPage) <= 1)
                          .map((p, idx, arr) => {
                              ...
                              <React.Fragment key={p}>
                                ...
                                <button
                                  ...
                                  style={{
                                    ...
                                    borderRadius: '6px',
                                    border: '1px solid #CBD5E1',
                                    backgroundColor: pqPage === p ? '#2563EB' : '#ffffff',
                                    color: pqPage === p ? '#ffffff' : '#0F172A',
                                    fontWeight: '600',
                                    fontSize: '0.8rem',
                                    cursor: 'pointer'
                                  }}
                        ...
                        <button
                          className="btn btn-secondary btn-sm"
                          disabled={pqPage >= Math.ceil(pqTotal / pqLimit)}
                          onClick={() => setPqPage(p => p + 1)}
                          style={{ padding: '6px 12px', display: 'flex', alignItems: 'center', gap: '4px' }}
                        >
                    ...
                    {drillModal.loading ? (
                      <tr>
                        <td colSpan={9} style={{ padding: '60px 0', textAlign: 'center' }}>
                          ...
                          <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                            <RefreshCw className="animate-spin" size={32} style={{ color: '#2563EB', animation: 'spin 1s linear infinite' }} />
                            <span style={{ fontSize: '0.9rem', color: '#64748B', fontWeight: '600' }}>Loading user details, please wait...</span>
                          </div>
                        </td>
                      </tr>
                    ) : (
                      <>
                        {drillModal.complaints.map(c => {
                          const isExpanded = drillExpandedRow === c.complaint_id;
                          return (
                            <React.Fragment key={c.id}>
                              <tr
                                onClick={() => setDrillExpandedRow(isExpanded ? null : c.complaint_id)}
                                style={{ cursor: 'pointer', backgroundColor: isExpanded ? '#F8FAFC' : '#ffffff', transition: 'background-color 0.2s' }}
                              >
                                ...
                                <td>{c.mobile ? maskPhoneNumber(c.mobile) : <span style={{ color: 'var(--text-light)' }}>N/A</span>}</td>
                                <td>W-{c.ward_number}</td>
                                <td>
                                  <span style={{
                                    color: c.priority === 'Urgent' || c.priority === 'CRITICAL' ? 'var(--color-danger)' : c.priority === 'High' || c.priority === 'HIGH' ? 'var(--color-warning)' : 'inherit',
                                    fontWeight: '600'
                                  }}>{c.priority}</span>
                                </td>
                                <td>
                                  <span className="badge" style={{
                                    ...
                                    fontSize: '0.75rem'
                                  }}>{c.status === 'Resolved' ? 'Closed' : c.status}</span>
                                </td>
                                ...
                                <tr style={{ backgroundColor: '#F8FAFC' }}>
                                  <td colSpan={9} style={{ padding: '12px 24px' }}>
                                    <div style={{
                                      ...
                                      <div style={{ gridColumn: '1 / -1' }}>
                                        <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '4px' }}>Full Description</div>
                                        <div style={{ fontSize: '0.85rem', color: '#0F172A', lineHeight: '1.4', whiteSpace: 'pre-wrap' }}>{c.description || 'No description provided.'}</div>
                                      </div>
                                      <div>
                                        <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '4px' }}>Citizen Name</div>
                                        <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#1E293B' }}>{c.citizen_name || 'N/A'}</div>
                                      </div>
                                      <div>
                                        <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '4px' }}>Mobile Number</div>
                                        <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#1E293B' }}>{maskPhoneNumber(c.mobile)}</div>
                                      </div>
                                      <div>
                                        <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '4px' }}>Date Raised</div>
                                        <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#1E293B' }}>
                                          {c.created_at ? new Date(c.created_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                                        </div>
                                      </div>
                                      <div>
                                        <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748B', textTransform: 'uppercase', marginBottom: '4px' }}>Last Updated</div>
                                        <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#1E293B' }}>
                                          {c.resolved_at ? new Date(c.resolved_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                                        </div>
                                      </div>
                                    </div>
                                  </td>
